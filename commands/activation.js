...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'فتح') {
      ...
      let period;
      try {
        period = await queue.openPeriod({ channelId: channel.id, maxTickets });
      } catch (err) {
    ...
    if (sub === 'اغلاق') {
      ...
      await queue.closePeriod(period._id);
      await interaction.reply({ content: `🔒 تم إغلاق استقبال تذاكر جديدة للفترة \`${period._id}\`.`, ephemeral: true });
    ...
    if (sub === 'حالة') {
      ...
      const summary = await queue.getQueueSummary(period._id);
      await interaction.reply({ embeds: [buildQueueStatusEmbed(period, summary)], ephemeral: true });
    }
  },
};
