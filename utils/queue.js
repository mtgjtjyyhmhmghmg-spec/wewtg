...
/** تسجيل استدعاء جديد — يرجع الطابع الزمني (snapshot) عشان نقارن فيه بعد 10 دقايق */
function setSummoned(ticketId, staffId) {
  db.prepare(`UPDATE tickets SET summoned_at = datetime('now'), summoned_by = ? WHERE id = ?`).run(staffId, ticketId);
  return getTicket(ticketId).summoned_at;
}

/** يُستدعى لما العضو يرد بروم تكته — يلغي الاستدعاء النشط */
function clearSummon(ticketId) {
  db.prepare(`UPDATE tickets SET summoned_at = NULL, summoned_by = NULL WHERE id = ?`).run(ticketId);
}

/**
 * يتحقق هل لسا نفس الاستدعاء نشط بعد المهلة (يعني العضو ما رد ولا صار استدعاء أحدث).
 * snapshot هو القيمة اللي رجعتها setSummoned وقت الاستدعاء.
 */
function isSummonStillPending(ticketId, snapshot) {
  const ticket = getTicket(ticketId);
  if (!ticket) return false;
  if (ticket.status === 'closed') return false;
  return ticket.summoned_at === snapshot;
}
